...
#
# Description:
#   Backup a MongoDB database (no auth), zip it, and email it via SendGrid.
...
log INFO "Starting MongoDB backup for database '$MONGO_DB_NAME'"
...
log INFO "Running: mongodump --host $MONGO_HOST --port $MONGO_PORT --db $MONGO_DB_NAME --out $DUMP_DIR"
    ...
    --output "$SENDGRID_RESPONSE_FILE" <<EOF
{
  ...
  "content": [{
    "type": "text/html",
    "value": "<!DOCTYPE html><html><body style='margin:0; padding:0; background:#f4f5f7; font-family:Arial, sans-serif;'><div style='max-width:600px; margin:40px auto; background:#ffffff; border-radius:12px; box-shadow:0 4px 16px rgba(15,23,42,0.12); overflow:hidden;'><div style='padding:20px 24px; background:#0f172a; color:#ffffff;'><h1 style='margin:0; font-size:18px;'>MongoDB Backup Completed</h1><p style='margin:4px 0 0 0; font-size:13px; opacity:0.85;'>Automated backup notification</p></div><div style='padding:24px 28px;'><p style='font-size:14px; color:#111827; margin-top:0;'>A new backup has been created for the database <strong>$MONGO_DB_NAME</strong>.</p><table style='width:100%; border-collapse:collapse; margin:16px 0; font-size:13px;'><tr><td style='padding:6px 0; color:#6b7280; width:120px;'>Database</td><td style='padding:6px 0; color:#111827;'><strong>$MONGO_DB_NAME</strong></td></tr><tr><td style='padding:6px 0; color:#6b7280;'>Created at</td><td style='padding:6px 0; color:#111827;'><strong>$DATE</strong></td></tr><tr><td style='padding:6px 0; color:#6b7280;'>Host</td><td style='padding:6px 0; color:#111827;'>$MONGO_HOST:$MONGO_PORT</td></tr></table><p style='font-size:13px; color:#4b5563; line-height:1.6;'>The backup is attached as a ZIP file. Store it securely and do not share it over untrusted channels.</p><div style='margin-top:20px; padding:12px 16px; border-radius:8px; background:#eff6ff; border:1px solid #dbeafe; font-size:12px; color:#1e3a8a;'><strong>Tip:</strong> Consider mirroring this backup to off-site storage (S3, GCS, etc.) for additional resilience.</div></div><div style='padding:14px 24px; background:#f9fafb; font-size:11px; color:#9ca3af; text-align:center;'>This email was generated automatically by the MongoDB backup script.<br/>If you did not expect this message, please rotate your credentials.</div></div></body></html>"
  }],
  "attachments": [{
...
# Delete all old ZIP backups, keep only the latest one
...
# Delete all SendGrid response JSON files
